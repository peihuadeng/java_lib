...
import com.dph.akkaDemo.FaultHandlingDocSample.CounterApi.UseStorage;
import com.dph.akkaDemo.FaultHandlingDocSample.CounterServiceApi.CurrentCount;
import com.dph.akkaDemo.FaultHandlingDocSample.CounterServiceApi.Increment;
import com.dph.akkaDemo.FaultHandlingDocSample.CounterServiceApi.ServiceUnavailable;
import com.dph.akkaDemo.FaultHandlingDocSample.StorageApi.Entry;
import com.dph.akkaDemo.FaultHandlingDocSample.StorageApi.Get;
import com.dph.akkaDemo.FaultHandlingDocSample.StorageApi.StorageException;
import com.dph.akkaDemo.FaultHandlingDocSample.StorageApi.Store;
import com.dph.akkaDemo.FaultHandlingDocSample.WorkerApi.Progress;
...
public class FaultHandlingDocSample {
	...
	/***
	 * 
	 * Worker performs some work when it receives the Start message. It will* continuously notify the sender of the Start message of current Progress.* The Worker supervise the CounterService.
	 */
	...
	public static class Worker extends UntypedActor {
		...
		public void onReceive(Object msg) {
			...
			if (msg.equals(WorkerApi.Start) && progressListener == null) {
				...
				getContext().system().scheduler().schedule(Duration.Zero(), Duration.create(1, "second"), getSelf(), WorkerApi.Do, getContext().dispatcher(), null);
			} else if (msg.equals(WorkerApi.Do)) {
				counterService.tell(new Increment(1), getSelf());
				counterService.tell(new Increment(1), getSelf());
				counterService.tell(new Increment(1), getSelf());
				// Send current progress to the initial sender
				pipe(ask(counterService, CounterServiceApi.GetCurrentCount, askTimeout).mapTo(classTag(CurrentCount.class)).map(new Mapper<CurrentCount, Progress>() {
	...
	/***
	 * 
	 * Adds the value received in Increment message to a persistent counter.* Replies with CurrentCount when it is asked for CurrentCount.CounterService* supervise Storage and Counter.
	 */
	...
	public static class CounterService extends UntypedActor {
		...
		/***
		 * 
		 * The child storage is restarted in case of failure, but after 3 restarts,* and still failing it will be stopped. Better to back- off than* continuously failing. When it has been stopped we will schedule a* Reconnect after a delay. Watch the child so we receive Terminated message* when it
		 * has been terminated.
		 */
	...
	/***
	 * 
	 * Saves key/ value pairs to persistent storage when receiving Store message.* Replies with current value when receiving Get message.Will throw*StorageException if the underlying data store is out of order.
	 */
